var utmparse_test = (function () {
	...
	var _testdata = [
		...
		//*		
		// Test: [input, [expected utm], hint, comment]
		...
		["0498237 7769796", [32, 0498237, 7769796], _hint],
		["33v -16765 6726243", [32, 313322, 6694142], _hint],
		["33v 6726243 -16765", [32, 313322, 6694142], _hint],
		["123 321", [32, 512300, 6632100],undefined, 'Should fail: Shorthand without hint'],
		["123,321", [32, 512300, 6632100], _hint, 'Shorthand'],
		["123 234", [32, 612300, 6723400], "32V 599999, 6699999", ''],
		["987 911", [32, 498700, 6591100], "32V 510123, 6610123", ''],
		["000 000", [32, 600000, 6700000], _hint, 'Shorthand: 000 000'],
		["7032813 0565179", [32, 0565179, 7032813], _hint, ''],
		["7032813, 0565179", [32, 0565179, 7032813], _hint, ''],
		["7032813,0565179", [32, 0565179, 7032813], _hint, 'Special case. No spaces, just comma'],
		["", [0,0,0]]];
	...
	_self.createTableRow = function (testdata, index) {
		...
		if (tokens.length == 1 && tokens[0].value.includes(',')) {
			tokens = geoparse.tokenizeString(teststr.replace(',', ' '), options);
		}
		...
		if (attempts == null) {
			...
			if (_SKIP) {
				div.append("<p>SKIPPING <i>utmtest()</i></p>");			
					...
					// Column Hint
					var hintStr = (hint && hint.utm) ? 'Hint: '+hint.utm.toString() : '';
					html += '<td>'+hintStr+"</td>\n";
		...
		html = "<tr>"+html+"</tr>";
		if (diff || _DISPLAY_TOKENS || showTokens) {
			var tmp = $(html);
